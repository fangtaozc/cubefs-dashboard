...
<template>
  <div class="outer-card">
    <el-row>
      <el-button
        v-if="$route.query.vid &&$route.query.disk_id"
        type="text"
        size="mini"
        class="back"
        @click="goBack"
...
export default {
  ...
  methods: {
    ...
    goBack() {
      ...
      this.$router.go(-1)
      this.$emit('activeChange', 'dataBlock')
    },
    goOrderDetail(url) {
      window.open(url)
    },
    async getData() {
      ...
      if (this.$route.query.disk_id) {
        for (const key in nodes) {
          this.data = nodes[key].disks
        }
      } else {
        let arr = []
        for (const host in nodes) {
          const disks = nodes[host]?.disks
          arr = arr.concat(disks)
        }
        this.data = arr.filter(item => {
          return this.diskArr.includes(item.disk_id)
        })
      }
    },
    ...
    toDataBlock(id, type) {
      this.$router.replace({ query: { ...this.$route.query, vid: id } })
      this.$emit('activeChange', 'dataBlock')
    },
  },
}
